...
Point::Point(const std::string & str_json, const size_t & _id) {
	id=_id;
	json j = json::parse(str_json);
	id_json=j.at("id").get<size_t>();
	name=j.at("name").get<string>();
	stimeFixe=j.at("stimeFixe").get<long>();
	capa_user=j.at("capa_user").get<int>();
	capa_vehi=j.at("capa_vehi").get<int>();
