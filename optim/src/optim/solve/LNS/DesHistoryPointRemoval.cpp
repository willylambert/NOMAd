...
#include "DesHistoryPointRemoval.h"
...
HistoryPointRemoval::HistoryPointRemoval(std::string s, double minprobability, double maxprobability,int nb_points):ADestroyOperator(s,minprobability,maxprobability) {
	//probability=0.85;
	p=6;
	/* Initialize the array with the students*/
	nbpoints=nb_points;
	scores = new double*[nb_points];
	for (int i = 0; i < nb_points; i++) {
		scores[i]= new double[nb_points];
	}
	for (int i = 0; i < nb_points; ++i) {
...
HistoryPointRemoval::~HistoryPointRemoval() {
...
void HistoryPointRemoval::destroySolution(Solution& sol) {
...
void HistoryPointRemoval::update(Solution& sol){
	Solution* darpsol= &sol;
	weights.clear();
	//weights.resize(0);
	...
	for (size_t idT = 0; idT < darpsol->getNbRoutes(); ++idT) {
		...
		for (auto node=(++tour->nodes.begin()), end=(--tour->nodes.end()); node!=end; ++node ) {
			auto previous=node;
			--previous;
			size_t i=(*previous)->point->getId();
			size_t j=(*node)->point->getId();
			double old_score=scores[i][j];
			scores[i][j]= min(old_score, score);
			if ((*node)->isOutType()) { // if pickup
