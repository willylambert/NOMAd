...
class ErrorHandler extends BaseObject{
  ...
  */
  public function __invoke($request, $response, $e) {
    ...
    // Log handled errors as warnings, others as errors
    if($e instanceof \OSS\AppException){
      $msg = "APP ERROR : " . $e->getMessage() . " @" . $e->getFile() . "[" . $e->getLine() . "] (Exception)";
      $this->log()->warn(array("message"=>$msg));
    }else{
