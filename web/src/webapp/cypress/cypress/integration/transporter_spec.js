...
context('transporter', () => {
  ...
  beforeEach(() => {
    cy.login();
    cy.navigateToList("data","transporters");
  })

  it('should check transporter list page ', () => {
    // Check the page title
    cy.get('legend > div').first().should('contain','Transporteurs');
    // Check that at least one transporter is already present in the list
    cy.get('tr>td>a').its('length').should('be.greaterThan',0)
    // check the presence of the button for adding a new transporter
    cy.get('#btn-add').should('exist');
  })

  /**
   * Create a new transporter. In the current version the transporter reference and label are the only parameter that can be changed.
   * @param string reference : the transporter reference
   * @param string label : the transporter label
   */
  function createNewTransporter(reference,label){
    cy.get('tr>td>a').contains(reference).should('not.exist')

    // Reach the page for a new transporter creation
    cy.get('#btn-add').click();
    cy.get('legend').should('contain','Nouveau transporteur');
    ...
    cy.get('.leaflet-control-geocoder-form >input').type('40 avenue édouard aynard ecully')
    cy.get('.leaflet-control-geocoder-form >input').type('{enter}')
    // the adress typed below is precise enough, we should not have some proposed alternatives, the marker
    //   corresponding to the found location should be put directly on the map.
    ...
    cy.get('.h5').should('contain','Transporteur '+reference);
  }

  it('should create a new transporter', () => {
    createNewTransporter('Transporteur_test','Transporteur test')
  })

  /**
   * Assuming we are in the transporter list menu, navigate to a transporter crud
   * @param string reference : the transporter reference
   */
  ...
  it('should update an existing transporter', () => {
    // Retrieve the test transporter
    navigateToCrud('Transporteur_test');
    // Enter edit mode
    cy.get('#btn-edit').click()
    // Change the description
    cy.get(':nth-child(3) > .col-md-10 > input').clear().type('Transporteur test...');
    cy.get('#btn-save').should('not.be.disabled')
    cy.get('#btn-save').click();
  ...
  it('should check changes in a transporter', () => {
    // Retrieve the test transporter
    navigateToCrud('Transporteur_test');
    // Enter edit mode
    cy.get('#btn-edit').click()
  ...
  it('should hard delete an existing transporter', () => {
    // Retrieve the test transporter
    navigateToCrud('Transporteur_test');
    ...
    cy.get('.modal-footer .btn-success').click()
    // Back in the list menu, check that the transporter was removed
    cy.get('legend > div > :nth-child(1)').should('contain','Transporteurs');
    cy.get('tr>td>a').contains('Transporteur_test').should('not.exist')
  });

  it('should mark a transporter as removed', () => {
    // Create a random number between 10000 and 19999
    var reference = 'test_'+(10000+Math.floor((Math.random() * 10000)));
    createNewTransporter(reference,'Random test transporter');
    // Retrieve the test transporter
    cy.navigateToList("data","transporters");
