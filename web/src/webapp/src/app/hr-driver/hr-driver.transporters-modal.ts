...
@Component({
  templateUrl: './hr-driver.transporters-modal.html',
})
export class HRTransportersModal {
    ...
    constructor(private siteService: SiteService, public activeModal: NgbActiveModal) {
      this.siteService.list({
          typeCode:"TRANSPORTER",statusCode:"ENABLED",search:"",startIndex:0,length:0}).subscribe((sites) => {
        this.transportersFormatted=[];
  ...
  gridReady(params){
    ...
    this.gridOptions.api.forEachNode(node=> {
      ...
      for(let transporter of this.transporters){
        if(node.data.code == transporter.code){
          selected = true;
          break;
        }
      }
    ...
    public insertSite(site){
      this.transportersFormatted.push({
        id:site.id,
        code:site.code,
        label:site.label,
  ...
  saveModal() {
    ...
    this.gridOptions.api.forEachNode(node=> {
      if(node.isSelected()){
        //get the transporter data
        this.siteService.get(node.data.id).subscribe(response=>this.transporters.push(response as Site))
      }
    });
    // Send the list of establishments as a response
    this.activeModal.close(this.transporters);
  }
}